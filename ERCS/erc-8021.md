...
author: Conner Swenberg (@ilikesymmetry), Dan Cortes (@dgca), Ryan Morning (@ryanmorning)
...
Schema ID `2` uses CBOR encoding for extensible transaction annotation:
...
| `r` | Custom registries - sub-map keyed by entity type (`a` for application, `w` for wallet), each containing `c` (chainId) and `a` (address) |
...
{
  "r": {
    "a": { "c": <chainId>, "a": "<address>" },
    "w": { "c": <chainId>, "a": "<address>" }
  }
}
```
...
- cborData: {"a": "bc_mnip", "w": "privy", "r": {"a": {"c": 8453, "a": "0xBcf2B9598..."}}, "m": {"utm_campaign": "winter-promo"}}
- builderCode: "bc_mnip"
- walletCode: "privy"
- codeRegistryChainId: 8453
