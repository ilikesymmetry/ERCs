...
author: Conner Swenberg (@ilikesymmetry), Dan Cortes (@dgca), Ryan Morning (@0xClouds)
...
1. **cborLength** (2 bytes): Length of the CBOR-encoded map in bytes (big-endian, always 2 bytes regardless of value)
2. **cborData** (variable length): CBOR-encoded map containing attribution data

The CBOR map MUST use the following key schema. All top-level keys are reserved for this specification.

| Key | Description                                                                                                                                                                   |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `a` | Application code - string value                                                                                                                                               |
| `w` | Wallet code - string value                                                                                                                                                    |
| `r` | Custom registries - sub-map keyed by entity type (`a` for application, `w` for wallet), each containing `c` (chainId as integer) and `a` (20-byte hex-encoded address string) |
| `m` | Custom metadata - sub-map of arbitrary key-value pairs                                                                                                                        |

The `r` (registry) field structure:

```json
{
  "r": {
    "a": { "c": <chainId (integer)>, "a": "<20-byte hex address>" },
    "w": { "c": <chainId (integer)>, "a": "<20-byte hex address>" }
  }
}
```

Example: `{"r": {"a": {"c": 8453, "a": "0xBcf2B9598Ec0781eE49230CaACF0FB3C881B5195"}}}`

Any other data included in the schema data MUST be included within the `m` (metadata) field. All other top-level keys are reserved and MUST NOT be used for purposes other than those defined above or in future iterations of the standard.
...
```
{txData}{cborData}{0x000c}{2}{0x80218021802180218021802180218021}
```
...
```
{txData}{cborData}{0x0085}{2}{0x80218021802180218021802180218021}
```
...
5. If `schemaId` is `2`:
   a. Extract the previous 2 bytes as `cborLength` (big-endian)
   b. Extract the previous `cborLength` bytes as `cborData`
   c. Decode `cborData` as a CBOR map
   d. Extract builder code from key `a`
...
```
Input: 0xdddddddd{cborData}000c0280218021802180218021802180218021
Expected:
- txData: 0xdddddddd
- schemaId: 2
- cborLength: 0x000c (12 bytes, 2-byte big-endian)
- cborData: {"a": "baseapp"}
- builderCode: "baseapp"
- registry: canonical
```
...
```
Input: 0xdddddddd{cborData}00850280218021802180218021802180218021
Expected:
- txData: 0xdddddddd
- schemaId: 2
- cborLength: 0x0085 (133 bytes, 2-byte big-endian)
- cborData: {"a": "baseapp", "w": "privy", "r": {"a": {"c": 8453, "a": "0xBcf2B9598..."}}, "m": {"utm_campaign": "winter-promo"}}
- builderCode: "baseapp"
- walletCode: "privy"
- codeRegistryChainId: 8453
- codeRegistryAddress: 0xBcf2B9598...
- metadata: {"utm_campaign": "winter-promo"}
```
