...
| `r` | Custom registries - sub-map keyed by entity type (`a` for application, `w` for wallet, `s` for services), each containing `c` (chainId as hex string) and `a` (20-byte hex-encoded address string). For services (`s`), the value is a sub-map keyed by the numeric index (as integer) of the corresponding entry in the `s` array, where each value contains `c` (chainId) and `a` (address) |
...

{
  ...
  "r": {
    "s": {
      ...
      1: { "c": "0x2105", "a": "0x1234567890abcdef1234567890abcdef12345678" }
    }
  }
}
```
...
5. If `schemaId` is `2`:
   ...
   g. Extract custom registries from key `r` (if present), containing sub-maps keyed by `a` (application), `w` (wallet), and/or `s` (services). For `a` and `w`, each value contains `c` (chainId) and `a` (address). For `s`, the value is a sub-map keyed by the numeric index into the `s` array, where each value contains `c` (chainId) and `a` (address)
   h. Extract metadata from key `m` (if present)
   i. For each code, if no custom registry specified for that entity type (or for services, that specific index), use chain's canonical Code Registry
