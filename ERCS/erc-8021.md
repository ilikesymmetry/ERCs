...
| `r` | Custom registries - sub-map keyed by entity type (`a` for application, `w` for wallet, `s` for services), each containing `c` (chainId as hex string) and `a` (20-byte hex-encoded address string). For `s`, the value is a sub-map keyed by the zero-based index into the `s` codes array, where each entry contains `c` and `a` for that service's registry |
...
Multiple entities with services and custom service registries (Schema ID 2):

```
{txData}{cborData}{cborLength}{2}{0x80218021802180218021802180218021}

{
  ...
  "r": {
    "s": {
      0: { "c": "0x2105", "a": "0xBcf2B9598Ec0781eE49230CaACF0FB3C881B5195" },
      1: { "c": "0x2105", "a": "0x9876543210abcdef9876543210abcdef98765432" }
    }
  }
}
```
...
5. If `schemaId` is `2`:
   ...
   g. Extract custom registries from key `r` (if present), containing sub-maps keyed by `a` (application), `w` (wallet), and/or `s` (services), each with `c` (chainId) and `a` (address). For `s`, the value is a sub-map keyed by zero-based index into the `s` codes array, where each entry contains `c` and `a` for that service's registry
   h. Extract metadata from key `m` (if present)
   i. For each code, if no custom registry specified for that entity type (or index for services), use chain's canonical Code Registry
...
Attribution with services and custom service registries (Schema ID 2):

```
Input: 
Expected:
- txData: 0xdddddddd
- schemaId: 2
- cborLength: 0x00a0 (160 bytes)
- cborData: {"a":"baseapp","w":"privy","s":["flashbots","titan"],"r":{"s":{"0":{"c":"0x2105","a":"0xBcf2B9598Ec0781eE49230CaACF0FB3C881B5195"},"1":{"c":"0x2105","a":"0x9876543210abcdef9876543210abcdef98765432"}}}}
```
